...
long long decimalToBasek(int decimalNumber,int k)
{
    long long rem, i = 1, Number = 0;
...
struct tree *  tree_generation(){
  ...
  //cout<<"\nd="<<d;
  long long var=decimalToBasek(target,segment);  //change accuracy value for the test cases
  //cout<<"\nvar="<<var;
 ...
 // cout<<"\n Parent node is created";
  ...
  var=target * 1e12;
  if(var%10==12)  //for removing error while multipling target with 1e9
    var+=1;
...
int main(){
    ...
    for(int a=2;a<=2;a++){ // This is to iterate over testcase folder
         for(int b=10;b<=10;b++){ //this is to iterate over testcase files with number of targets
             for(int c=5;c<=5;c++){
                  waste1=0.0,reactant1=0.0,buffer1=0.0,total_waste=0.0,total_reactant=0.0,total_buffer=0.0;
                  tms1=0,total_tms=0;
                  string str1="N"+to_string(a)+"/n"+to_string(a)+"_d"+to_string(b)+"/Test_d"+to_string(b)+"_n"+to_string(a)+"_"+to_string(file_arr[c])+".txt";
                  string str2="N"+to_string(a)+"/n"+to_string(a)+"_d"+to_string(b)+"/Result_d"+to_string(b)+"_n"+to_string(a)+"_"+to_string(file_arr[c])+".txt";
                  freopen(str1.c_str(),"r",stdin);
                  freopen(str2.c_str(),"w",stdout);

                  //freopen("N2/n2_d10/Test_d10_n2_10.txt","r",stdin);
                  //freopen("N2/n2_d10/Result_d10_n2_10.txt","w",stdout);
                  int total=0;
