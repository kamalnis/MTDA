...
struct tree *  tree_generation(){
 ...
 // cout<<"\n Parent node is created";
  target=(float)var/pow((float)10,segment);
...
void operations(struct tree *root){
  ...
  //cout<<"\ntms increased and node val="<<root->val;
    for(int i=0;i<root->adjacent.size();i++){
      ...
      else if(counter.find(node->val*1e6)!=counter.end()){
...
int main(){
    ...
    //freopen("Test_d64_100.txt","r",stdin);
    //freopen("Result_Test_d64_100.txt","w",stdout);
    int n;
    ...
    for(int i=0;i<n;i++){
      ...
      /* for(map<int,int>::iterator it=counter.begin();it!=counter.end();it++){
       ...
       t.clear();
       counter.clear();
       cout<<endl<<tms1<<" "<<reactant1<<" "<<buffer1<<" "<<waste1;
